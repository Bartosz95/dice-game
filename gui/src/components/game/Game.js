...
export default () => {
  ...
  const markDiceToRoll = (diceID) => {
    if (game.numberOfRoll === 0 || game.numberOfRoll === 3) return;

    const newDicesToChange = dicesToChange.includes(diceID)
      ? dicesToChange.filter((dice) => dice !== diceID)
      : dicesToChange.concat(diceID);
  ...
  const setupNewMug = (body) => {
    ...
    setGame((game) => {
      return { ...game, mug, numberOfRoll: body.game.numberOfRoll };
    });
    setDicesToChange([]);
    setChosenFigure(null);
    setNotReload(false);
  ...
  const dicesDiv =
    !(game.numberOfTurn === 0 && game.numberOfRoll === 0) &&
    game.mug.map((dice) => (
      <Dice
        key={dice.id}
        dice_props={dice}
