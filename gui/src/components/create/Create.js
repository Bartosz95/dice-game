...
export default () => {
  ...
  useEffect(() => {
    if (keycloak.authenticated && userInfo.sub) {
      fetchData(
        {
          url: `${keycloak.authServerUrl}/admin/realms/${keycloak.realm}/users`,
        },
        (body) => setUsers(body.filter((user) => user.id !== userInfo.sub))
      );
    }
  ...
  const selectUser = (selectedUser) => {
    ...
    setUsers((users) =>
      users.map((user) =>
        user.id === selectedUser.id ? toggleUser(user) : user
      )
    );
  };
  ...
  const createGame = () => {
    ...
    fetchData(requestOptions, (body) =>
      navigate(`/games/${body._id}`, { replace: false })
    );
  };
